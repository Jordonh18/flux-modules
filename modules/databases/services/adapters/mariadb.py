"""
MariaDB 11 Database Adapter

Complete adapter implementation for MariaDB 11.
Provides container configuration, health checks, metrics collection,
backup/restore, and database/user management operations.

MariaDB is a MySQL fork with compatible wire protocol and similar administration,
but uses MARIADB_ prefixed environment variables for configuration.
"""
...
class MariaDBAdapter(BaseAdapter):
    """MariaDB 11 database engine adapter."""
    ...
    def get_container_config(
        ...
        """
        Generate MariaDB container configuration.
        ...
        Note: MariaDB uses MARIADB_ prefix for environment variables, not MYSQL_.
        """
        env_vars = {}
        env_file_vars = {}
        ...
        if secrets_paths and "root_password" in secrets_paths:
            env_file_vars["MARIADB_ROOT_PASSWORD"] = secrets_paths["root_password"]
        else:
            env_vars["MARIADB_ROOT_PASSWORD"] = password

        # Application user configuration
        if username != "root":
            env_vars["MARIADB_USER"] = username
            if secrets_paths and "user_password" in secrets_paths:
                env_file_vars["MARIADB_PASSWORD"] = secrets_paths["user_password"]
            else:
                env_vars["MARIADB_PASSWORD"] = password

        # Initial database
        if database_name:
            env_vars["MARIADB_DATABASE"] = database_name
        ...
        # MariaDB needs specific capabilities for user management
        capabilities = [
            "SETGID",
            "SETUID",
            "CHOWN",
    ...
    def get_health_check_command(self, username: str, password: str) -> list[str]:
        ...
        MariaDB includes mysqladmin utility compatible with MySQL.
        """
        return [
            "mysqladmin",
            "ping",
    ...
    def parse_health_check_output(
        ...
        if returncode == 0 and "mysqld is alive" in stdout:
            return HealthStatus(
                healthy=True,
                status="healthy",
                message="MariaDB server is responding to ping",
            )
        elif returncode != 0 and "Can't connect" in stderr:
            return HealthStatus(
                healthy=False,
                status="unhealthy",
                message="MariaDB server is not accepting connections",
            )
        else:
            return HealthStatus(
                healthy=False,
    ...
    def get_metrics_command(
        ...
        """
        Generate MariaDB metrics collection command.
        ...
        SELECT JSON_OBJECT(
            ...
            'innodb_buffer_pool_reads', (SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_STATUS WHERE VARIABLE_NAME = 'Innodb_buffer_pool_reads'),
            'innodb_buffer_pool_read_requests', (SELECT VARIABLE_VALUE FROM information_schema.GLOBAL_STATUS WHERE VARIABLE_NAME = 'Innodb_buffer_pool_read_requests')
        ) AS metrics;
        """
    ...
    def parse_metrics_output(self, stdout: str) -> MetricsData:
        """
        Parse JSON metrics output from MariaDB.
    ...
    def get_backup_command(
        ...
        """
        Generate mariadb-dump backup command.
    ...
    def get_restore_command(
        ...
        """
        Generate MariaDB restore command.

        Pipes the backup file into the mariadb CLI client.
        """
        cmd = [
            "mariadb",
            "-u",
    ...
    def get_create_database_command(
        ...
        """Create a new MariaDB database with UTF-8 encoding."""
        return [
            "mariadb",
            "-u",
            username,
    ...
    def get_list_users_command(self, username: str, password: str) -> list[str]:
        ...
        FROM mysql.user 
        WHERE User NOT IN ('mysql.sys', 'mysql.session', 'mysql.infoschema', 'root', 'mariadb.sys')
        AND User != ''
        ORDER BY User;
        """
        return [
    ...
    def get_connection_string(
        ...
        """Generate a MariaDB connection URI (uses mysql:// protocol)."""
        return f"mysql://{username}:{password}@{host}:{port}/{database}"

    def get_log_parser_type(self) -> str:
        """MariaDB uses MySQL-compatible log format."""
        return "mysql"
    ...
    def get_volume_mounts(self, volume_paths: dict[str, str]) -> dict[str, str]:
        """Map data volume to MariaDB's data directory."""
        mounts = {}
        if "data" in volume_paths:
    ...
    def get_startup_probe_delay(self) -> int:
        """
        MariaDB takes longer to initialize on first start.
