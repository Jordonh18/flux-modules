"""
KeyDB Database Adapter

Complete adapter implementation for KeyDB (multithreaded Redis-compatible).
KeyDB is a high-performance fork of Redis with multi-threading support.
"""
...
class KeyDBAdapter(BaseAdapter):
    """KeyDB (multithreaded Redis-compatible) adapter."""
    ...
    def get_container_config(
        ...
        """Generate container configuration for KeyDB."""
        command = []
        ...
        if secrets_paths and "root_password" in secrets_paths:
            ...
            command = [
                "sh", "-c",
                "keydb-server --requirepass $(cat /secrets/root_password) --server-threads 2"
            ]
        else:
    ...
    def get_health_check_command(self, username: str, password: str) -> list[str]:
        """Return health check command for KeyDB."""
        return [
            "keydb-cli",
            "-a", password,
            "--no-auth-warning",
    ...
    def parse_health_check_output(self, returncode: int, stdout: str, stderr: str) -> HealthStatus:
        """Parse KeyDB PING output."""
        if returncode == 0 and "PONG" in stdout.upper():
            return HealthStatus(
                healthy=True,
                status="healthy",
                message="KeyDB is responding to PING",
        ...
        return HealthStatus(
            healthy=False,
            status="unhealthy",
            message=f"KeyDB health check failed: {stderr or stdout}",
    ...
    def get_metrics_command(self, database_name: str, username: str, password: str) -> list[str]:
        """Return metrics collection command for KeyDB."""
        return [
            "keydb-cli",
            "-a", password,
            "--no-auth-warning",
    ...
    def parse_metrics_output(self, stdout: str) -> MetricsData:
        """Parse KeyDB INFO output (same format as Redis)."""
        metrics = MetricsData()
    ...
    def get_backup_command(
        ...
        """Return KeyDB backup command using BGSAVE."""
        return [
            "keydb-cli",
            "-a", password,
            "--no-auth-warning",
    ...
    def get_restore_command(
        ...
        """Return KeyDB restore command (requires shutdown)."""
        return [
            "keydb-cli",
            "-a", password,
            "--no-auth-warning",
